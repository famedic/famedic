...
class EnsureUserHasAdminAccount
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        $personalNavItem = $request->user()->administrator->hasPermissionTo('administrators.manage') ? [
        ...
        $operationItems = array_values(array_filter([
            ...
            $request->user()->administrator->hasPermissionTo('customers.manage') ? [
                ...
                'items' => [
                    ...
                    [
                        'label' => 'Referenciados',
                        'url' => route('admin.customers.referrals'),
                        'current' => Route::currentRouteName() === 'admin.customers.referrals',
                    ],
                ],
            ] : null,
        ...
        // Quitar grupos vacíos (sin items y sin url).
        $operationItems = array_values(array_filter(
            $operationItems,
        ...
        $workspaceNavItem = $canOpenWorkspace ? [
            'label' => 'Workspace',
            'icon' => 'CpuChipIcon',
            'emoji' => '🧠',
            'badge' => 'BETA',
        ...
        $legalDocumentationNavItem = $request->user()->administrator->hasPermissionTo('documentation.manage') ? [
        ...
        $monitoringTopItems = array_values(array_filter([
        ...
        $monitoringSidebarItems = array_values(array_filter([
        ...
        $adminMembershipsNavItem = ($request->user()->administrator->roles()->where('roles.id', 1)->exists() || $canViewOdessaReconciliation || $canViewOdessaPreEnrollments) ? [
        ...
        $adminItems = array_values(array_filter([
            $monitoringSidebarItems !== [] ? [
                'label' => 'Monitoreo',
                'icon' => 'ClipboardDocumentListIcon',
                'items' => $monitoringSidebarItems,
            ] : null,
        ]));
            ...
            'adminTopNavigation' => [
                ...
                'configuration' => array_values(array_filter([
                    ...
                    $this->adminHasPermission($administrator, 'activecampaign.manage') ? [
                        ...
                        'icon' => 'SignalIcon',
                        'current' => Route::currentRouteName() === 'admin.activecampaign.settings',
                    ] : null,
                ])),
                'memberships' => $adminMembershipsNavItem,
                'monitoring' => $monitoringTopItems,
                'notifications' => array_values(array_filter([
                    $this->adminHasPermission($administrator, 'activecampaign.manage') ? [
                        'label' => 'Centro ActiveCampaign',
                        'url' => route('admin.activecampaign.notifications'),
                        'icon' => 'SignalIcon',
                        'current' => Route::currentRouteName() === 'admin.activecampaign.notifications',
                    ] : null,
                    $isAuthorizer ? [
                        'label' => $pendingAuthorizationsCount > 0
                            ? "Autorizaciones ({$pendingAuthorizationsCount})"
                            : 'Autorizaciones',
                        'url' => route('admin.coupons.authorizations.index'),
                        'icon' => 'BanknotesIcon',
                        'current' => str_starts_with((string) Route::currentRouteName(), 'admin.coupons.authorizations.'),
