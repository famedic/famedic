...
class QaBatchLaboratoryResultsCommandTest extends TestCase
{
    ...
    #[Test]
    public function batch_command_no_permite_production(): void
    {
        app()->detectEnvironment(fn () => 'production');

        $exitCode = Artisan::call('laboratory:results:qa-batch');
    ...
    #[Test]
    public function batch_no_publica_vision_ni_modifica_text_publicado(): void
    {
        app()->detectEnvironment(fn () => 'testing');

        $version = $this->seedVersion(LaboratoryResultsPdfFixture::binary(
        ...
        $exitCode = Artisan::call('laboratory:results:qa-batch', ['--limit' => 1]);
    ...
    #[Test]
    public function batch_genera_metricas_agregadas_y_por_documento(): void
    {
        app()->detectEnvironment(fn () => 'testing');

        $this->seedVersion(LaboratoryResultsPdfFixture::binary(
            LaboratoryResultsPdfFixture::standardResultsLines()
        ));
        ...
        $this->assertStringContainsString('Versions evaluated:', $output);
        $this->assertStringContainsString('Comparable pairs:', $output);
        $this->assertStringContainsString('SHADOW MODE', $output);
        $this->assertStringContainsString('version_id=', $output);
        $this->assertGreaterThanOrEqual(1, LaboratoryResultExtractionQaMetric::query()->count());
    }

    #[Test]
    public function output_no_contiene_pii(): void
    {
        app()->detectEnvironment(fn () => 'testing');
        ...
        Artisan::call('laboratory:results:qa-batch', ['--limit' => 1]);
        $output = Artisan::output();

        $this->assertStringNotContainsString('@test.local', $output);
        $this->assertStringNotContainsString('Paciente', $output);
        $this->assertStringNotContainsString('8112345678', $output);
    }

    #[Test]
    public function verbose_conflicts_off_no_imprime_valores_clinicos(): void
    {
        app()->detectEnvironment(fn () => 'testing');
        ...
        $this->assertStringNotContainsString('text_value=95', $output);
        $this->assertStringNotContainsString('vision_value=9.5', $output);
    }
    ...
    private function seedVersion(string $binary): LaboratoryResultVersion
    {
        ...
        $path = 'results/gda-'.$purchase->id.'-batch.pdf';
