...
function resolveActiveStepId(
	...
	if (usesAppointmentFirstFlow) {
		if (stepId === "confirmation") {
			stepId = laboratoryAppointment?.confirmed_at
				? "payment"
				: "appointment";
		}
...
function resolveInitialStepIndex(
	...
	try {
		...
		if (saved) {
			...
			const fromStorage = wizardSteps.findIndex(
				(s) => s.id === savedStepId,
			);
			if (fromStorage >= 0) {
				return fromStorage;
...
function resolveInitialCheckoutData(savedCheckout = null) {
	...
	return {
		contact: fromParams.contact ?? savedCheckout.contact_id ?? null,
		address: fromParams.address ?? savedCheckout.address_id ?? null,
		payment_method:
			fromParams.payment_method ?? savedCheckout.payment_method ?? null,
...
export default function LaboratoryCheckout({
	...
	const [appliedPromo, setAppliedPromo] = useState(() =>
		initialFormData.promo_validation_token
			? {
					...
					message:
						"Código promocional guardado. Valídalo de nuevo si cambió tu carrito.",
				}
			: null,
	);
	...
	const submit = (e, isBranchPayment) => {
		...
		if (isBranchPayment) {
			// PAGO EN SUCURSAL
			const items = laboratoryCarts[laboratoryBrand.value].map(
				(item) => ({
				...
				}),
				{
					onError: (errors) => {
						...
						if (errors.payment_method) {
							alert(
								`Error en método de pago: ${errors.payment_method}`,
							);
	...
	const minPurchaseShortfallCents = useMemo(() => {
		if (
			!selectedCoupon?.min_purchase_cents ||
			couponMeetsMinPurchase(selectedCoupon, total)
		) {
			return 0;
		}
	...
	const couponBlocked =
		couponTooLarge || couponNotYetValid || couponBelowMinPurchase;
	...
	const summaryDetails = useMemo(() => {
		...
		if (hasPromoApplied) {
			rows.push({
				...
				label: appliedPromo?.benefit_label || "Descuento promocional",
			});
		}
		rows.push({
			value:
	...
	const syncCheckoutDraft = useCallback(() => {
		...
		router.post(
			...
			payload,
			{
				...
				onError: (syncErrors) => {
					console.error(
						"No se pudo guardar el checkout:",
						syncErrors,
					);
	...
	const usesPayPalAtCheckout =
		hasPayPal && paypalClientId && data.payment_method === "paypal";
				...
				>
					{syncingDraft
						...
						: syncingAppointment
							...
							: currentStep.id === "appointment" &&
								  wizardLaboratoryAppointment?.confirmed_at &&
								  wizardLaboratoryAppointment?.is_payable !==
										false
								? "Continuar al pago"
								: currentStep.id === "appointment" &&
									  (wizardLaboratoryAppointment?.has_left_callback_info ||
											callbackPreferenceSavedAtFormatted)
									? "Esperando llamada"
									: currentStep.id === "appointment"
										? "Contacto pendiente"
						...
						<div className="flex flex-col gap-3">
							...
							{paymentUsesMock && (
								<Text className="text-sm text-amber-800/90 dark:text-amber-200/90">
									Modo pruebas: las tarjetas precargadas no
