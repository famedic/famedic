...
function ChannelCard({ icon: Icon, title, subtitle, checked, disabled, recommended = false, onSelect }) {
	return (
		<button
			type="button"
			onClick={onSelect}
...
export default function CouponAuthorizationOtpModal({
	isOpen,
	couponId,
	...
	const sendUrl = route("admin.coupons.authorizations.approval-otp.send", couponId);
	const verifyUrl = route("admin.coupons.authorizations.approval-otp.verify", couponId);
	...
	const submitSend = async () => {
		...
		setSending(true);
		try {
			const r = await jsonFetch(sendUrl, { method: "POST", body: otpBody() });
			...
			<div className="space-y-4">
				<div className="flex items-start gap-3">
					...
					<div className="min-w-0">
						<DialogTitle>Verificación para aprobar</DialogTitle>
				...
				<DialogBody className="space-y-4">
					<Text className="text-sm text-zinc-600 dark:text-slate-400">
						Confirma tu identidad con un código enviado a tu correo o SMS antes de registrar la
						aprobación.
					</Text>
				...
				<DialogActions>
					...
					{step === "channel" ? (
						...
						<Button
							...
							className="min-h-11 justify-center"
						>
							{verifying ? "Verificando..." : "Confirmar aprobación"}
						</Button>
					)}
				</DialogActions>
			</div>
