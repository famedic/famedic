...
export default function TagsExecutive({ executive = null }) {
	if (!executive) {
		return (
		...
		<section className="space-y-4">
			...
			<div className="grid gap-4 xl:grid-cols-2">
				<MiniBar
					...
					description="Eventos de dispatch mapeados al tag en el periodo."
					data={executive.most_used}
					color={DASHBOARD_COLORS.green}
				/>
				<MiniBar
					title="Tags sin uso"
					description="Inventario sin señal de uso en el periodo (muestra)."
					data={executive.unused}
					color={DASHBOARD_COLORS.orange}
				/>
				<ChartCard
					title="Distribución"
					description="Automáticos / manuales / con uso / sin uso."
				>
					{(executive.distribution || []).length ? (
						<div className="h-48">
							<ResponsiveContainer width="100%" height="100%">
								<BarChart data={executive.distribution}>
									<CartesianGrid strokeDasharray="3 3" opacity={0.3} />
									<XAxis dataKey="label" tick={{ fontSize: 11 }} />
									<YAxis allowDecimals={false} />
									<Tooltip />
				...
				<MiniBar
					title="Tendencia"
					description="Dispatches ligados a tags por día."
					data={executive.trend}
					color={DASHBOARD_COLORS.purple}
				/>
			</div>

			{(executive.by_origin || []).length ? (
				<MiniBar
					title="Por origen"
					description="ActiveCampaign / Config / Legacy."
					data={executive.by_origin}
				/>
			) : null}
