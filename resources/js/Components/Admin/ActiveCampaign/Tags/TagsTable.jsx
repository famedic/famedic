	...
	if (!tags.length) {
		return (
			<section className="space-y-3">
				...
				<EmptyListCard
					heading="Sin tags"
					message="No hay tags que coincidan con los filtros."
				/>
			</section>
		);
	}
		...
		<section className="space-y-3">
			<div>
				...
				<p className="text-xs text-zinc-500">
					{total} tag{total === 1 ? "" : "s"} · inventario compuesto
				</p>
			</div>
				...
				<Table bleed className="[--gutter:theme(spacing.6)]" dense>
					<TableHead>
						<TableRow>
							...
							<TableHeader>Contactos</TableHeader>
							<TableHeader className="hidden md:table-cell">Origen</TableHeader>
							<TableHeader>Automatización</TableHeader>
						...
						{tags.map((item) => {
							...
							return (
								...
								>
									...
									<TableCell>
										<Badge color={APP[item.application] || "zinc"}>
