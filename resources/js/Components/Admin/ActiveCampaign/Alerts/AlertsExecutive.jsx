...
export default function AlertsExecutive({ executive = null }) {
	if (!executive) {
		return (
	...
	return (
		<section className="space-y-4">
			<div className="flex flex-wrap items-center gap-2">
				...
				<AnalyticsTruthBadge truth={executive.truth || "proxy"} />
			</div>
			{executive.note ? (
			...
			<div className="grid gap-4 xl:grid-cols-2">
				<MiniBar
					...
					description="Volumen de alertas por categoría."
					data={executive.top_categories}
				/>
				<MiniBar
					title="Alertas por módulo"
					description="Distribución por módulo origen."
					data={executive.by_module}
				/>
				<MiniBar
					title="Por prioridad"
					description="Crítica / Alta / Media / Baja."
					data={executive.by_priority}
				/>
				<ChartCard
					title="Abiertas vs resueltas"
					description="Stock del periodo (sesión + señales)."
				>
					{(executive.open_vs_resolved || []).length ? (
						<div className="h-48">
							<ResponsiveContainer width="100%" height="100%">
								<BarChart data={executive.open_vs_resolved}>
									<CartesianGrid strokeDasharray="3 3" opacity={0.3} />
									<XAxis dataKey="label" />
									<YAxis allowDecimals={false} />
									<Tooltip />
