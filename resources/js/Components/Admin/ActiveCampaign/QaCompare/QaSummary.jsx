...
export default function QaSummary({ summary = [] }) {
	return (
		<section className="space-y-3">
			<div>
				...
				<p className="text-xs text-zinc-500">
					Señales del ambiente actual vs remoto no instrumentado — sin secretos.
				</p>
			</div>
				...
				{summary.map((card) => (
					<div key={card.id} className="relative">
						<div className="absolute right-3 top-3 z-10">
							<AnalyticsTruthBadge truth={card.truth || "proxy"} />
						</div>
						<BillingMetricCard
							label={card.label}
							value={card.value}
