...
export function DropdownItem({ className, ...props }) {
	let classes = clsx(
		className,
		// Base styles
		"group cursor-default rounded-lg px-3.5 py-2.5 focus:outline-none sm:px-3 sm:py-1.5 font-poppins",
		// Text styles
		"text-left text-base/6 text-zinc-950 sm:text-sm/6 dark:text-white forced-colors:text-[CanvasText]",
		// Focus
		"data-[focus]:bg-famedic-dark dark:data-[focus]:bg-white data-[focus]:text-white dark:data-[focus]:text-famedic-darker",
		// Disabled state
		...
		"[&>[data-slot=icon]]:text-zinc-500 [&>[data-slot=icon]]:data-[focus]:text-white [&>[data-slot=icon]]:dark:text-slate-400 [&>[data-slot=icon]]:data-[focus]:dark:text-famedic-darker",
		// Avatar
...
export function DropdownDivider({ className, ...props }) {
	return (
		<Headless.MenuSeparator
			...
			className={clsx(
				className,
				"col-span-full mx-3.5 my-1 h-px border-0 bg-zinc-950/5 sm:mx-3 dark:bg-slate-700 forced-colors:bg-[CanvasText]",
			)}
		/>
	);
}
...
export function DropdownDescription({ className, ...props }) {
	return (
		<Headless.Description
			...
			className={clsx(
				className,
				"col-span-2 col-start-2 row-start-2 text-sm/5 text-zinc-500 group-data-[focus]:text-white sm:text-xs/5 dark:text-slate-400 dark:group-data-[focus]:text-slate-800 forced-colors:group-data-[focus]:text-[HighlightText]",
			)}
		/>
	);
}
