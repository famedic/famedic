		...
		<Card className="min-w-0 max-w-full overflow-hidden rounded-2xl p-4 shadow-sm sm:p-6">
			...
			<div className="min-w-0 space-y-3 text-sm">
				...
				{totals.hasCatalogDiscount && (
					<Row
						label="Descuento"
						value={`−${totals.catalogDiscount}`}
					/>
				)}

				{totals.hasAppliedCreditBalance && totals.creditApplied && (
					<CreditRow amount={totals.creditApplied} />
				)}

				{totals.creditAppliedMessage && (
					<p className="rounded-lg border border-violet-200/80 bg-violet-50/60 px-3 py-2 text-xs leading-snug text-violet-900 dark:border-violet-800/40 dark:bg-violet-950/25 dark:text-violet-100">
						{totals.creditAppliedMessage}
					</p>
				)}

				<Row label="Total" value={totals.netTotal} strong />

				<div className="flex min-w-0 flex-wrap items-center justify-between gap-x-3 gap-y-2">
					<span className="shrink-0 text-zinc-500 dark:text-slate-400">
						Método de pago
					</span>
...
function Row({ label, value, strong = false }) {
	return (
		<div className="flex min-w-0 flex-wrap items-baseline justify-between gap-x-3 gap-y-1">
			...
			<span
				className={`min-w-0 max-w-full text-right break-words tabular-nums ${
					strong
...
function CreditRow({ amount }) {
	return (
		<div className="flex min-w-0 flex-wrap items-center justify-between gap-x-3 gap-y-1">
			<span className="inline-flex shrink-0 items-center gap-1.5 text-zinc-500 dark:text-slate-400">
				<GiftIcon
					className="size-4 shrink-0 fill-violet-600 dark:fill-violet-400"
					aria-hidden
				/>
				Crédito a favor
			</span>
